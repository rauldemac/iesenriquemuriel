...
if (!isset($_REQUEST['enviar'])){
	...
	if ($mysqli=mysqli_connect("localhost","root")){
		if($mysqli->select_db("test")){
			$query='insert into registro values (NULL, "'.$_REQUEST['nombre'].'", "'.$_REQUEST['apellidos'].'", "'.$_REQUEST['password'].'", "'.$_REQUEST['email'].'", "'.$_REQUEST['gender'].'", "'.$_REQUEST['foto'].'", "'.$_REQUEST['residencia'].'", "'.$_REQUEST['dia'].'", "'.$_REQUEST['mes'].'", "'.$_REQUEST['año'].'", "'.@$_REQUEST['nivel'].'", "'.$_REQUEST['boletin1'].'", "'.$_REQUEST['boletin2'].'")';
